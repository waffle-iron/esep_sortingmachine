...
void Test::actuatorsTest(){
	...
	_hal->motorStart();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_START));
	_hal->motorRotateCounterclockwise();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_ROTATE_COUNTER_CLOCKWISE));
	_hal->motorFast();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_FAST));
	...
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_ROTATE_COUNTER_CLOCKWISE));
	_hal->motorSlow();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_SLOW));
	if( !nextTest(__FUNCTION__) ) return;
	...
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_ROTATE_CLOCKWISE));
	_hal->motorFast();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_FAST));
	if( !nextTest(__FUNCTION__) ) return;
	...
	_hal->motorStop();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::MOTOR_STOP));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test open switch point - please continue soon" << endl;
	_hal->switchPointOpen();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::SWITCH_OPEN));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test close switch point " << endl;
	_hal->switchPointClose();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::SWITCH_CLOSE));
...
void Test::mmiTest(){
	...
	_hal->Q1LEDOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::Q1_LED_ON));
	_hal->Q2LEDOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::Q2_LED_ON));
	_hal->ResetLEDOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::RESET_LED_ON));
	_hal->StartLEDOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::START_LED_ON));
	if( !nextTest(__FUNCTION__) ) return;

	cout<< "testButtonLEDs off: Reset,Start,Q1,Q2" <<endl;
	_hal->Q1LEDOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::Q1_LED_OFF));
	_hal->Q2LEDOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::Q2_LED_OFF));
	_hal->ResetLEDOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::RESET_LED_OFF));
	_hal->StartLEDOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::START_LED_OFF));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test lamps on: red, yellow, green" << endl;
	_hal->yellowLightOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::YELLOW_LIGHT_ON));
	_hal->redLightOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::RED_LIGHT_ON));
	_hal->greenLightOn();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::GREEN_LIGHT_ON));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test blinking fast: red, yellow, green " << endl;
	_hal->blinkGreen(Speed::fast);
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::BLINK_GREEN_FAST));
	_hal->blinkRed(Speed::fast);
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::BLINK_RED_FAST));
	_hal->blinkYellow(Speed::fast);
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::BLINK_YELLOW_FAST));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test blinking fast: yellow, green" << endl;
	_hal->redLightOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::RED_LIGHT_OFF));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test blinking fast: green" << endl;
	_hal->yellowLightOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::YELLOW_LIGHT_OFF));
	if( !nextTest(__FUNCTION__) ) return;

	cout << "test blinking slow: red, yellow, green" << endl;
	_hal->blinkGreen(Speed::slow);
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::BLINK_GREEN_SLOW));
	_hal->blinkYellow(Speed::slow);
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::BLINK_YELLOW_SLOW));
	_hal->blinkRed(Speed::slow);
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::BLINK_RED_SLOW));
	if( !nextTest(__FUNCTION__) ) return;
	...
	_hal->greenLightOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::GREEN_LIGHT_OFF));
	_hal->yellowLightOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::YELLOW_LIGHT_OFF));
	_hal->redLightOff();
	_hal->sendSerial(Signal(cb_this, 0xff, Signalname::RED_LIGHT_OFF));
